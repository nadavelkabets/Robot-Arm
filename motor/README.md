...
- When a motor is accelerated, the voltage of the "back EMF" rises, and so the overall voltage is lower. (=supplied voltage - back EMF voltage).
- Less current can be supplied. For that reason, the faster the motor goes the less torque it is able to produce. The maximal speed could only be achieved without load.
