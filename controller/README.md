...
The most popular circuit to operate a 3 phase motor is a 3 half-bridge system. each half bridge consists of 2 MOSFETs - one connected to V+ and one to GND. The source side of both MOSFETs is connected and forms a bridge. This way, by switching each side on and of, we can controll the voltage on each phase using PWM and turn on or off the phase.
