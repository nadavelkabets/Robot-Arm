...
#tofix - add error messages for user
...
class actuator:
    ...
    def check_state(odrive_unit, odrive_axis):
        return odrive_unit.odrive_axis.current_state

    def request_state(odrive_unit, odrive_axis, state):
        odrive_unit.odrive_axis.requested_state = state
...
actuator0 = actuator(odrive_unit = "odrv0", odrive_axis = "axis0", gear_ratio = 50)
...
def startup():
    ...
    for actu in actuator.list:
        actu.request_state(actu.odrive_unit, actu.odrive_axis, state = 3)

     #checks if calibration is complete
        for actu in actuator.list:
            while actu.check_state(actu.odrive_unit, actu.odrive_axis) != 3:
                time.sleep(0.1)
            actu.request_state(actu.odrive_unit, actu.odrive_axis, state = 8)
